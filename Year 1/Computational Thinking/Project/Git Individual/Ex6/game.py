...
def list_of_items(items):
    ...
    for item in items:
        n = item["name"]
        items_as_string += (f"{n}, ")
    items_as_string = items_as_string[0: len(items_as_string) - 2]
...
def print_menu(exits, room_items, inv_items):
    ...
    # Iterate over possible items to take from room
    for item in room_items:
        print("TAKE", item["id"].upper(), "to take a", item["name"])

    # Iterate over items to drop
...
def execute_go(direction):
    ...
    if (player.current_room["exits"], direction):
        ...
        new_room = move(c_room["exits"], direction)
...
def execute_command(command):
    ...
    if command[0] == "go":
        if len(command) > 1:
            if(is_valid_exit(player.current_room["exits"], command[1]) == True):
